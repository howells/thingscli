...
vi.mock("@howells/cli", () => ({
  ...
  hardenId: (value: string, command: string, options?: { label?: string }) => {
    ...
    // Checked by code point rather than a regex literal: this mock stands in
    // for @howells/cli's hardenId, and a control-character class inside a
    // regex is exactly what noControlCharactersInRegex rejects.
    if ([...value].some((char) => char.charCodeAt(0) <= 0x1f)) {
      throw new Error(
        `[${command}] Invalid ${label}: contains control characters.`,
      );
    }
    if (value.includes("..") || value.includes("/") || value.includes("\\")) {
      throw new Error(
        `[${command}] Invalid ${label}: contains path traversal characters.`,
      );
    }
  },
}));
...
describe("validateUuid", () => {
  ...
  it("rejects control characters", () => {
    expect(() => validateUuid("abc123def456ghi789\x00kl", "test")).toThrow(
      "Invalid",
    );
  });

  it("rejects slashes", () => {
    expect(() => validateUuid("abc123def456ghi789/kl0", "test")).toThrow(
      "Invalid",
    );
  });
});
