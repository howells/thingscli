...
// Every failure this CLI reports goes out as structured JSON through
// error(). Opening the Things database is the one that did not: a machine
// without Things - every non-Mac included - got a raw ENOENT stack trace
// from readdirSync instead. Catching at the dispatch boundary keeps the
// contract for anything thrown below it, not just this one case.
try {
  switch (command) {
    case "today":
    ...
    case "update": {
      ...
      const opts = {
        ...
        deadline,
        addTags:
          (json["add-tags"] as string) ??
          (json.addTags as string) ??
          flag("add-tags"),
        notes: (json.notes as string) ?? flag("notes"),
        appendNotes:
          (json["append-notes"] as string) ??
          (json.appendNotes as string) ??
          flag("append-notes"),
      ...
      if (hasFlag("dry-run")) {
        success(
          { action: "update", ...opts, authToken: "[redacted]" },
          "update",
        );
      }
    ...
    case "-h":
      success(
        {
          usage:
            "thingscli <command> [args] [--fields uuid,title] [--limit 10] [--json '{...}'] [--dry-run]",
          read: [
            "today",
            "inbox",
            "upcoming",
          ...
          flags: {
            ...
            "--dry-run":
              "Validate and return payload without executing (writes only)",
  ...
  error(err instanceof Error ? err.message : String(err), command);
}
